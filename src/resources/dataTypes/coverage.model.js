...
const codeableConceptSchema = require('./codeableConcept.model')
const identifierSchema = require('./identifier.model')
const metaSchema = require('./meta.model')
const periodSchema = require('./period.model')
const referenceSchema = require('./reference.model')
const moneySchema = require('./money.model')
...
const coverageSchema = Schema(
  {
    ...
    }, // R!  active | cancelled | draft | entered-in-error
    _status: {
      type: String,
      enum: ['active', 'cancelled', 'draft', 'entered-in-error']
    }, // R!  active | cancelled | draft | entered-in-error
    ...
    _subscriberId: { type: String }, // ID assigned to the subscriber
    beneficiary: { type: referenceSchema }, // R!  Plan beneficiary
    dependent: { type: String }, // Dependent number
    _dependent: { type: String }, // Dependent number
    ...
    _order: { type: String, match: /^[1-9][0-9]*$/g }, // Relative order of the coverage
    network: { type: String }, // Insurer network
    _network: { type: String }, // Insurer network
    costToBeneficiary: [
      {
    ...
    _subrogation: { type: Boolean }, // Reimbursement to insurer
